...
by checking the only known external consumer (the downstream renderer, now `serum-render`), which imports
...
- `examples/render_preset.py` (delete as duplicate) — at the time of this audit, README documented this as the no-install-required entry point; intentional duplication. (File has since moved to the downstream renderer repo in commit fdda99e.)
