...
**Reason:** Two findings in this run proposed removing `juce_memoryblock_b64decode` (promoted to public API in commit 9fd668a) and `examples/render_preset.py` (documented in README as the no-install-required entry point; the example was later moved to the vst-render repo in commit fdda99e). In both cases the "is this actually used?" question that drove the audit finding has already been answered deliberately by the author; re-flagging adds noise without surfacing new information.
